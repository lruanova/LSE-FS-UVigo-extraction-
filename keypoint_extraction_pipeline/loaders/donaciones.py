...
class DonacionesDataset(GeneratorBasedBuilder):
    def _info(self) -> DatasetInfo:
        """Returns the dataset metadata as a DatasetInfo object."""

        return DatasetInfo(
            description="Donaciones dataset from Signamed project.",
            features=Features(
                {
                    "segment_id": Value("string"),  # unique ID per annotation
                    "video_path": Value("string"),  # file path
    ...
    def _generate_examples(
        ...
        try:
            for idx, annotation in enumerate(parser.parse(csv_path)):
                ann_id = f"don_{idx}"
                ann = {
                    "segment_id": f"don_{split_name}_{idx}",
